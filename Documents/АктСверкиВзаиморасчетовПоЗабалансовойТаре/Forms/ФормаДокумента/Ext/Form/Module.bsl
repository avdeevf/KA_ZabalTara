...
Функция ЗаполнитьПоДаннымБухгалтерскогоУчета(ЗаполнятьДанныеКонтрагента)
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне(
		"Документы.АктСверкиВзаиморасчетовПоЗабалансовойТаре.ПодготовитьДанныеДляЗаполнения",
		СтруктураПараметров,
		ПараметрыВыполнения);
...
Процедура ИзменитьДоступностьПоСогласованиюСверки(Форма)
	...
	Элементы.ОстатокНаНачалоКво.ТолькоПросмотр	= Объект.СверкаСогласована;
...
Процедура ЗаполнитьСчетамиПоУмолчаниюНаСервере()
	...
	ТаблицаСчетов = УчетВзаиморасчетов.ПолучитьТаблицуСчетовУчетаВзаиморасчетовЗаб(Истина,Истина);
...
Процедура ПересчитатьОстатки(Форма)
	...
	Форма.ОстатокНаКонецКво = Объект.ОстатокНаНачалоКво
		+ Объект.ПоДаннымОрганизации.Итог("КоличествоДт")
		- Объект.ПоДаннымОрганизации.Итог("КоличествоКт");
	...
	Форма.ОстатокНаНачалоКонтрагентКво = - Объект.ОстатокНаНачалоКво;
	Форма.ОстатокНаКонецКонтрагентКво = Форма.ОстатокНаНачалоКонтрагентКво
		+ Объект.ПоДаннымКонтрагента.Итог("КоличествоДт")
		- Объект.ПоДаннымКонтрагента.Итог("КоличествоКт");

	Объект.Расхождение = Форма.ОстатокНаКонец + Форма.ОстатокНаКонецКонтрагент;
	Объект.РасхождениеКво = Форма.ОстатокНаКонецКво + Форма.ОстатокНаКонецКонтрагентКво;
	Форма.РасхождениеКонтрагент = - Объект.Расхождение;
	Форма.РасхождениеКонтрагентКво = - Объект.РасхождениеКво;
...
Процедура ПересчитатьОстаткиНаСервере()
	...
	ОстатокНаКонецКво = Объект.ОстатокНаНачалоКво + Объект.ПоДаннымОрганизации.Итог("КоличествоДт") -  Объект.ПоДаннымОрганизации.Итог("КоличествоКт");
	ОстатокНаНачалоКонтрагентКво = - Объект.ОстатокНаНачалоКво;
	...
	ОстатокНаКонецКонтрагентКво = ОстатокНаНачалоКонтрагентКво + Объект.ПоДаннымКонтрагента.Итог("КоличествоДт") -  Объект.ПоДаннымКонтрагента.Итог("КоличествоКт");
	...
	КонецЕсли; 
	Если Объект.РасхождениеКво <> ОстатокНаКонецКво + ОстатокНаКонецКонтрагентКво Тогда
		Объект.РасхождениеКво = ОстатокНаКонецКво + ОстатокНаКонецКонтрагентКво;
	КонецЕсли; 
	
	Расхождение = Объект.Расхождение;
	РасхождениеКонтрагент = - Расхождение;
	РасхождениеКво = Объект.РасхождениеКво;
	РасхождениеКонтрагентКво = - РасхождениеКво;
...
Процедура ЗаполнитьДанныеКонтрагентаПоДаннымОрганизацииСервер()
	...
	СоответствиеПредставлений = Документы.АктСверкиВзаиморасчетовПоЗабалансовойТаре.СоответствиеПредставленийПоДаннымКонтрагента(КодЯзыкаПечать);
...
Функция ЗагрузитьПодготовленныеДанные(Результат)
	...
	Если СтруктураДанных.Успешно Тогда
		...
		Объект.ОстатокНаНачалоКво = СтруктураДанных.ОстатокНаНачалоКво;
