...
Процедура ДобавитьКомандыОтправки(КомандыОтправки) Экспорт
	...
	КомандаОтправки.Идентификатор = "АктСверкиЗаб";
	КомандаОтправки.Представление = НСтр("ru='Акт сверки (заб.)';uk='Акт звірки (заб.)'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "АктСверкиЗаб";
	КомандаПечати.Представление = НСтр("ru='Акт сверки (заб.)';uk='Акт звірки (заб.)'");
	...
	КомандаПечати.ЗаголовокФормы = НСтр("ru='Реестр документов ""Акт сверки расчетов с контрагентом (заб.)""';uk='Реєстр документів ""Акт звірки розрахунків з контрагентом (заб.)""'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктСверкиЗаб") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "АктСверкиЗаб", НСтр("ru='Акт сверки взаиморасчетов (заб.)';uk='Акт звірки взаєморозрахунків (заб.)'"),
		ПечатьАктаСверки(МассивОбъектов, ОбъектыПечати,ПараметрыВывода), , "Документ.АктСверкиВзаиморасчетовПоЗабалансовойТаре.ПФ_MXL_АктСверки", , Истина);
	КонецЕсли;
...
|	ТЧПоДаннымОрганизации.КоличествоДт КАК КоличествоДтОрг,
|	ТЧПоДаннымОрганизации.Кредит КАК Кредит,
|	ТЧПоДаннымОрганизации.КоличествоКт КАК КоличествоКтОрг,
|	NULL КАК ПредставлениеКонтр,
|	NULL КАК ДатаКонтр,
|	0 КАК ДебетКонтр,
|	0 КАК КоличествоДтКонтр,
|	0 КАК КредитКонтр,
|	0 КАК КоличествоКтКонтр
|ПОМЕСТИТЬ ДанныеДокумента
|ИЗ
|	Документ.АктСверкиВзаиморасчетовПоЗабалансовойТаре.ПоДаннымОрганизации КАК ТЧПоДаннымОрганизации
|ГДЕ
...
|	ТЧПоДаннымКонтрагента.КоличествоДт,
|	ТЧПоДаннымКонтрагента.Кредит,
|	ТЧПоДаннымКонтрагента.КоличествоКт
|ИЗ
|	Документ.АктСверкиВзаиморасчетовПоЗабалансовойТаре.ПоДаннымКонтрагента КАК ТЧПоДаннымКонтрагента
|ГДЕ
...
|////////////////////////////////////////////////////////////////////////////////
|ВЫБРАТЬ
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.Ссылка КАК Документ,
...
|	ЕСТЬNULL(ДанныеДокумента.КоличествоДтОрг, 0) КАК КвоОборотДт,
|	ЕСТЬNULL(ДанныеДокумента.Кредит, 0) КАК СуммаОборотКт,
|	ЕСТЬNULL(ДанныеДокумента.КоличествоКтОрг, 0) КАК КвоОборотКт,
|	ЕСТЬNULL(ДанныеДокумента.ДебетКонтр, 0) КАК СуммаОборотДтКонтр,
|	ЕСТЬNULL(ДанныеДокумента.КоличествоДтКонтр, 0) КАК КвоОборотДтКонтр,
|	ЕСТЬNULL(ДанныеДокумента.КредитКонтр, 0) КАК СуммаОборотКтКонтр,
|	ЕСТЬNULL(ДанныеДокумента.КоличествоКтКонтр, 0) КАК КвоОборотКтКонтр,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.Организация КАК Организация,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.Контрагент КАК Контрагент,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.Дата КАК ДатаАктаСверки,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ПредставительОрганизации КАК ПредставительОрганизации,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ПредставительКонтрагента КАК ПредставительКонтрагента,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ДатаНачала КАК ДатаНачала,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ДатаОкончания КАК ДатаОкончания,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ОстатокНаНачало КАК ОстатокНаНачало,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ОстатокНаНачалоКво КАК ОстатокНаНачалоКво,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.Расхождение КАК Расхождение,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ДоговорКонтрагента КАК ДоговорКонтрагентаВШапке,
|	ЕСТЬNULL(АктСверкиВзаиморасчетовПоЗабалансовойТаре.ДоговорКонтрагента.ЭтоГруппа, ЛОЖЬ) КАК ДоговорКонтрагентаВШапкеЭтоГруппа,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ДоговорКонтрагента КАК ДоговорКонтрагента,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ДоговорКонтрагента.Дата КАК ДоговорДата,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ДоговорКонтрагента.Номер КАК ДоговорНомер,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ДоговорКонтрагента.НаименованиеДляПечати КАК ДоговорНаименованиеДляПечати,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.ВалютаДокумента КАК ВалютаДокумента,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.РазбитьПоДоговорам КАК РазбитьПоДоговорам,
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.СверкаСогласована КАК СверкаСогласована,
|	ДанныеДокумента.НомерТабЧасти КАК НомерТабЧасти
|ИЗ
|	Документ.АктСверкиВзаиморасчетовПоЗабалансовойТаре КАК АктСверкиВзаиморасчетовПоЗабалансовойТаре
|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеДокумента КАК ДанныеДокумента
|		ПО (ДанныеДокумента.Документ = АктСверкиВзаиморасчетовПоЗабалансовойТаре.Ссылка)
|ГДЕ
|	АктСверкиВзаиморасчетовПоЗабалансовойТаре.Ссылка В (&МассивДокументов)
|
|УПОРЯДОЧИТЬ ПО
|	НомерТабЧасти,
...
|	СУММА(КвоОборотДтКонтр),
|	СУММА(СуммаОборотКтКонтр),
|	СУММА(КвоОборотКтКонтр),
...
Функция ПечатьАктаСверки(МассивОбъектов, ОбъектыПечати,ПараметрыВывода)
	...
	ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_АктСверкиВзаиморасчетовПоЗабалансовойТаре";
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.АктСверкиВзаиморасчетовПоЗабалансовойТаре.ПФ_MXL_АктСверки");
	...
	Пока Шапка.Следующий() Цикл
		...
		КвоОстатокНаНачало 	= Шапка.Документ.ОстатокНаНачалоКво;
		...
		ПараметрыНачальныеОстатки.Вставить("КвоНачальныйОстатокДт", 	?(Шапка.ОстатокНаНачалоКво > 0, Шапка.ОстатокНаНачалоКво, 0));
		ПараметрыНачальныеОстатки.Вставить("СуммаНачальныйОстатокКт", 	?(Шапка.ОстатокНаНачало < 0, -Шапка.ОстатокНаНачало, 0));
		ПараметрыНачальныеОстатки.Вставить("КвоНачальныйОстатокКт", 	?(Шапка.ОстатокНаНачалоКво < 0, -Шапка.ОстатокНаНачалоКво, 0));
		...
		Пока ОбходПоДоговорам.Следующий() Цикл
			...
			Если Шапка.РазбитьПоДоговорам И ЕстьСтрокиДокументов Тогда
				...
				СтруктураСальдоПоДоговору.Вставить("НачОстатокПоДоговоруДт", НачальноеСальдоПоДоговору.Итог("НачОстатокПоДоговоруДт"));
				СтруктураСальдоПоДоговору.Вставить("КвоНачОстатокПоДоговоруДт", НачальноеСальдоПоДоговору.Итог("КвоНачОстатокПоДоговоруДт"));
				СтруктураСальдоПоДоговору.Вставить("НачОстатокПоДоговоруКт", НачальноеСальдоПоДоговору.Итог("НачОстатокПоДоговоруКт"));
				СтруктураСальдоПоДоговору.Вставить("КвоНачОстатокПоДоговоруКт", НачальноеСальдоПоДоговору.Итог("КвоНачОстатокПоДоговоруКт"));
			...
			Если Шапка.РазбитьПоДоговорам И ЕстьСтрокиДокументов Тогда
				...
				КвоКонОстатокПоДоговоруДт = СтруктураСальдоПоДоговору.КвоНачОстатокПоДоговоруДт + ОбходПоДоговорам.КвоОборотДт;
				КонОстатокПоДоговоруКт = СтруктураСальдоПоДоговору.НачОстатокПоДоговоруКт + ОбходПоДоговорам.СуммаОборотКт;
				КвоКонОстатокПоДоговоруКт = СтруктураСальдоПоДоговору.КвоНачОстатокПоДоговоруКт + ОбходПоДоговорам.КвоОборотКт;
				КонОстатокПоДоговору   = КонОстатокПоДоговоруДт - КонОстатокПоДоговоруКт;
				КвоКонОстатокПоДоговору   = КвоКонОстатокПоДоговоруДт - КвоКонОстатокПоДоговоруКт;
				...
				КонецЕсли;
				Если КвоКонОстатокПоДоговору < 0 Тогда
					СтруктураКонСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруДт", 0);
					СтруктураКонСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруКт",-КвоКонОстатокПоДоговору);
				Иначе
					СтруктураКонСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруДт", КвоКонОстатокПоДоговору);
					СтруктураКонСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруКт", 0);
				КонецЕсли;
				
				Если НЕ ЗаполнятьДанныеКонтрагента Тогда
					СтруктураКонСальдоПоДоговору.Вставить("КонОстатокПоДоговоруДтКонтр", 0);
					СтруктураКонСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруДтКонтр", 0);
					СтруктураКонСальдоПоДоговору.Вставить("КонОстатокПоДоговоруКтКонтр", 0);
					СтруктураКонСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруКтКонтр", 0);
				Иначе
					КонОстатокПоДоговоруДтКонтр = СтруктураСальдоПоДоговору.НачОстатокПоДоговоруКт + ОбходПоДоговорам.СуммаОборотДтКонтр;
					КвоКонОстатокПоДоговоруДтКонтр = СтруктураСальдоПоДоговору.КвоНачОстатокПоДоговоруКт + ОбходПоДоговорам.КвоОборотДтКонтр;
					КонОстатокПоДоговоруКтКонтр = СтруктураСальдоПоДоговору.НачОстатокПоДоговоруДт + ОбходПоДоговорам.СуммаОборотКтКонтр;
					КвоКонОстатокПоДоговоруКтКонтр = СтруктураСальдоПоДоговору.КвоНачОстатокПоДоговоруДт + ОбходПоДоговорам.КвоОборотКтКонтр;
					КонОстатокПоДоговоруКонтр = КонОстатокПоДоговоруДтКонтр - КонОстатокПоДоговоруКтКонтр;
					КвоКонОстатокПоДоговоруКонтр = КвоКонОстатокПоДоговоруДтКонтр - КвоКонОстатокПоДоговоруКтКонтр;
					...
					Иначе
						...
						СтруктураКонСальдоПоДоговору.Вставить("КонОстатокПоДоговоруКтКонтр", 0);
					КонецЕсли;
					Если КвоКонОстатокПоДоговоруКонтр < 0 Тогда
						СтруктураКонСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруДтКонтр", 0);
						СтруктураКонСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруКтКонтр",-КвоКонОстатокПоДоговоруКонтр);
					Иначе
						СтруктураКонСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруДтКонтр", КвоКонОстатокПоДоговоруКонтр);
						СтруктураКонСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруКтКонтр", 0);
					КонецЕсли;
				КонецЕсли;
		...
		КвоОстатокНаКонец = Шапка.ОстатокНаНачалоКво + Шапка.КвоОборотДт - Шапка.КвоОборотКт;
		Если ЗаполнятьДанныеКонтрагента Тогда
			...
			КвоОстатокНаКонецКонтр = Шапка.ОстатокНаНачалоКво - Шапка.КвоОборотДтКонтр + Шапка.КвоОборотКтКонтр;
		Иначе
			ОстатокНаКонецКонтр = 0;
			КвоОстатокНаКонецКонтр = 0;
		КонецЕсли;
		...
		ПараметрыИтогиПоКонтрагенту.Вставить("КвоКонечныйОстатокДт",      	?(КвоОстатокНаКонец > 0, КвоОстатокНаКонец, 0));
		ПараметрыИтогиПоКонтрагенту.Вставить("СуммаКонечныйОстатокКт",      ?(ОстатокНаКонец < 0, -ОстатокНаКонец, 0));
		ПараметрыИтогиПоКонтрагенту.Вставить("КвоКонечныйОстатокКт",      	?(КвоОстатокНаКонец < 0, -КвоОстатокНаКонец, 0));
		ПараметрыИтогиПоКонтрагенту.Вставить("СуммаКонечныйОстатокКтКонтр", ?(ОстатокНаКонецКонтр > 0, ОстатокНаКонецКонтр, 0));
		ПараметрыИтогиПоКонтрагенту.Вставить("КвоКонечныйОстатокКтКонтр", 	?(КвоОстатокНаКонецКонтр > 0, КвоОстатокНаКонецКонтр, 0));
		ПараметрыИтогиПоКонтрагенту.Вставить("СуммаКонечныйОстатокДтКонтр", ?(ОстатокНаКонецКонтр < 0, -ОстатокНаКонецКонтр, 0));
		ПараметрыИтогиПоКонтрагенту.Вставить("КвоКонечныйОстатокДтКонтр", 	?(КвоОстатокНаКонецКонтр < 0, -КвоОстатокНаКонецКонтр, 0));
...
Процедура ВывестиСальдоПоОстальнымДоговорам(НачальноеСальдоПоДоговорам, ОбластьДоговор, ОбластьДоговорИтоги, ТабличныйДокумент, Шапка)
	...
	Для Каждого СтрокаДоговора Из НачальноеСальдоПоДоговорам Цикл
		...
		КонечноеСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруДт",      СтрокаДоговора.КвоНачОстатокПоДоговоруДт);
		КонечноеСальдоПоДоговору.Вставить("КонОстатокПоДоговоруКт",      	СтрокаДоговора.НачОстатокПоДоговоруКт);
		КонечноеСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруКт",      СтрокаДоговора.КвоНачОстатокПоДоговоруКт);
		КонечноеСальдоПоДоговору.Вставить("КонОстатокПоДоговоруДтКонтр", 	СтрокаДоговора.НачОстатокПоДоговоруКт);
		КонечноеСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруДтКонтр", СтрокаДоговора.КвоНачОстатокПоДоговоруКт);
		КонечноеСальдоПоДоговору.Вставить("КонОстатокПоДоговоруКтКонтр", 	СтрокаДоговора.НачОстатокПоДоговоруДт);
		КонечноеСальдоПоДоговору.Вставить("КвоКонОстатокПоДоговоруКтКонтр", СтрокаДоговора.КвоНачОстатокПоДоговоруДт);
...
|	СУММА(ОстаткиПоДоговорамАкта.КоличествоОстатокДт) КАК КвоНачОстатокПоДоговоруДт,
|	СУММА(ОстаткиПоДоговорамАкта.КоличествоОстатокКт) КАК КвоНачОстатокПоДоговоруКт
|ИЗ
...
|	СУММА(ОстаткиПоОстальнымДоговорам.КоличествоОстатокДт) КАК КвоНачОстатокПоДоговоруДт,
|	СУММА(ОстаткиПоОстальнымДоговорам.КоличествоОстатокКт) КАК КвоНачОстатокПоДоговоруКт
|ИЗ
...
Функция НовыйСтруктураДокумента(ПоДаннымКонтрагента = Ложь)
	...
	СтруктураДокумента.Вставить("КвоОборотДт" 	+ Суффикс);
	СтруктураДокумента.Вставить("СуммаОборотКт" + Суффикс);
	СтруктураДокумента.Вставить("КвоОборотКт" 	+ Суффикс);
...
Функция НовыйТаблицаДокументов()
	...
	ТаблицаДокументов.Колонки.Добавить("КоличествоДт",  ОбщегоНазначения.ОписаниеТипаЧисло(15,2));
	ТаблицаДокументов.Колонки.Добавить("Дебет",         ОбщегоНазначения.ОписаниеТипаЧисло(15,2));
	ТаблицаДокументов.Колонки.Добавить("КоличествоКт",  ОбщегоНазначения.ОписаниеТипаЧисло(15,2));
...
Функция ПредставлениеХозяйственнойОперации(Выборка, КодЯзыкаПечать)
	...
	Если Выборка.Дебет > 0 Тогда
		Представление =  НСтр("ru='Поступление тары на забалансовый счет';uk='Надходження тари на забалансовий рахунок'",КодЯзыкаПечать);
	...
	ИначеЕсли Выборка.Кредит > 0 Тогда
		Представление = НСтр("ru='Передача тары с забалансового счета';uk='Передано тару з забалансового рахунку'",КодЯзыкаПечать);
	Иначе
...
Процедура ДополнитьТаблицуВалютнойСуммой(ТаблицаПоДаннымОрганизации, СтруктураПараметров)
	...
	Если СтруктураПараметров.ВалютаДокумента = СтруктураПараметров.ВалютаРегламентированногоУчета 
		...
		ТаблицаПоДаннымОрганизации.Свернуть(
			...
			"КоличествоДт, Дебет, КоличествоКт, Кредит, ВалютнаяСумма");
	...
	Иначе
		ТаблицаПоДаннымОрганизации.Свернуть("Дата,ДатаВходящегоДокумента,Документ,Представление,Договор","КоличествоДт, Дебет, КоличествоКт, Кредит");
 	КонецЕсли;
...
Функция ТаблицаПоДаннымКонтрагента(ТаблицаПоДаннымОрганизации,КодЯзыкаПечать,СтруктураПараметров)
	...
	Для Каждого СтрокаПоДаннымОрганизации Из ТаблицаПоДаннымОрганизации Цикл
		...
		КонецЕсли;	
		СтрокаПоДаннымКонтрагента.КоличествоДт  = СтрокаПоДаннымОрганизации.КоличествоКт;
		СтрокаПоДаннымКонтрагента.Дебет  		= СтрокаПоДаннымОрганизации.Кредит;
		СтрокаПоДаннымКонтрагента.КоличествоКт  = СтрокаПоДаннымОрганизации.КоличествоДт;
...
Функция ПодготовитьЗапрос(СтруктураПараметров)
	...
	|	ХозрасчетныйОстаткиНаНачало.КоличествоОстаток КАК КоличествоОстатокНаНачало
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(
	|			&ДатаНачала,
	|			Счет В
	...
	|			&АналитикаРасчетов,
	|			Организация = &Организация
	|				И Субконто1 В (&МассивКонтрагентов)) КАК ХозрасчетныйОстаткиНаНачало
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ХозрасчетныйОбороты.КоличествоОборотДт КАК КоличествоДт,
	|	ХозрасчетныйОбороты.КоличествоОборотКт КАК КоличествоКт
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(
	|			&ДатаНачала,
	|			&ДатаОкончания,
...
Процедура ПодготовитьДанныеДляЗаполнения(СтруктураПараметров, АдресХранилища) Экспорт
	...
	СтруктураДанныхЗаполнения.Вставить("ОстатокНаНачалоКво", ВыборкаОстатокНаНачало.КоличествоОстатокНаНачало);
